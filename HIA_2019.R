...
for(bound in bound_vec) {
  ...
  for(dis in dis_vec) {
    dis_walkers <- emp_long %>% filter(disease == dis)
...
################################################################################################################################
#                                   6. HIA OUTCOMES : Total of prevented cases, DALY, costs                                    #
################################################################################################################################
...
plot_no_dep_prev <- burden_sex %>%  
  mutate(disease = factor(disease, levels = c("mort", "cancer", "cvd", "diab2", "dem", "dep")))  %>% 
  filter(!disease %in% c("dep")) %>% 
  ggplot(aes(x = disease, y = tot_cases_mid, ymin = tot_cases_low, ymax = tot_cases_up, fill = sex)) +
  geom_bar(width = 0.7, position = position_dodge2(.7), stat = "identity")  +
  geom_errorbar(position = position_dodge(.7), width = .25) +
  scale_fill_manual(values = colors_sex) +
...
# Plot : DEPRESSION
plot_dep_prev <- burden_sex %>% filter(disease == "dep") %>% 
  ggplot(aes(x = disease, y = tot_cases_mid, ymin = tot_cases_low, ymax = tot_cases_up, fill = sex)) +
  geom_bar(width = 0.7, position = position_dodge2(.7), stat = "identity")  +
  geom_errorbar(position = position_dodge(.7), width = .25) +
  scale_fill_manual(values = colors_sex) +
