...
burden_target_area_total <- burden_target_area %>% 
  group_by(area_type) %>%
  summarise(
...
walk_2019 <- emp_main_walk_trip  %>% 
  ...
  mutate(step = pmin(12000, round(total_steps/ 10) * 10 + 2000))
...
for(bound in bound_vec) {
  ...
  for(dis in dis_vec) {
    dis_walkers <- walk_2019 %>% 
    filter(disease == dis)
    
    bound_list[[dis]] <- dis_walkers
  }
  
  walk_trips_2019_list[[bound]] <- bound_list
}
...
################################################################################################################################
# Calculate for each individual the number of prevented cases, DALY and costs
HIA_2019_list <- calc_HIA(data_list = walk_trips_2019_list,
                       rr_table = reduc_rr_table,
                       dw_table = dw_table,
                       dis_vec = dis_vec,
                       bound_vec = bound_vec)
...
##############################################################
# Total of prevented burden of each disease
burden_2019 <- burden_prevented(data_list = HIA_2019_list, 
                                  dis_vec = dis_vec,
                                  bound_vec = bound_vec,
                                  group = NULL)
...
##############################################################
# Prevented diseases per area type
burden_2019_area <- burden_prevented(data_list = HIA_2019_list, 
                                       dis_vec = dis_vec,
                                       bound_vec = bound_vec,
                                       group = "area_type")
...
# Total of health burden per area type
burden_2019_area_total <- burden_2019_area_order %>% 
  group_by(area_type) %>%
  summarise(
    across(where(is.numeric), \(x) sum(x, na.rm = TRUE)),
    .groups = "drop") %>% 
...
# Bind total to the previous table
burden_2019_area_global <- bind_rows(burden_2019_area_order, burden_2019_area_total) %>% 
  arrange(area_type, desc(tot_cases_mid)) %>% 
  mutate(disease = factor(disease, levels = unique(disease)))
...
plot_cases_prev_by_area <- burden_target_area_order %>% 
  ...
  labs(
    title = "Total cases prevented by walking - best practice scenario",
    x = "Territory type",
...
plot_cases_prev_by_age_area <- burden_target_area_age %>% 
  ...
  labs(
    title = "Cases prevented by walking - best practice scenario",
...
#                                                      8. DESCRIPTION                                                          #
################################################################################################################################
# Répartition des déplacements dans chaque type de territoire

# Répartition d'individus dans chaque territoire
...
# Plot 
ggsave(here("output", "Plots", "Best practice", "tot_cases_prevented.png"), plot = plot_cases_prev_by_area)
ggsave(here("output", "Plots", "Best practice", "cases_prevented_area.png"), plot = plot_cases_prev_by_area_disease)
ggsave(here("output", "Plots", "Best practice", "tot_cases_prevented_age.png"), plot = plot_cases_prev_by_age_area)
