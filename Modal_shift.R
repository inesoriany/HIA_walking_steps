...
drivers_dist <- emp_car_trip  %>% 
  filter(!is.na(nbkm_car) & nbkm_car <= dist)                      # Select the car trips under this distance
...
# Import 2019 data
disease_2019 <- import(here("output", "Tables", "2019", "Resampling", "HIA_per_disease.xlsx"))
...
plot_shift_cases_prev <- 
  ...
  # Modal shift
  geom_bar(data = disease_shift_IC %>%  
           filter(!disease %in% c("All", "Morbidity")), 
  ...
  geom_errorbar(data = disease_shift_IC %>%  
                filter(!disease %in% c("All", "Morbidity")),
