...
# Number of people who change their area type between resident commune and origin commune 
emp_walk_trips <- emp_walk_trips  %>% 
  left_join(emp_walkers %>% select(ident_ind, densitecom_res), by = "ident_ind")

area_ori_res_change <- emp_walk_trips  %>% 
  filter(densitecom_ori != densitecom_res)

nb_area_ori_res_change <- area_ori_res_change  %>% 
  filter(!is.na(pond_indc)) %>% 
  distinct(ident_ind, .keep_all = TRUE) %>%
  as_survey_design(ids = ident_ind,
                   weights = pond_indc)  %>% 
  summarise(nb_respondents = unweighted(n()),
            nb_walkers = survey_total(1, na.rm = TRUE))



# Number of people who change their area type between resident commune and arrival commune 
area_res_des_change <- emp_walk_trips  %>% 
  filter(densitecom_res != densitecom_des)

nb_area_res_des_change <- area_res_des_change  %>% 
  filter(!is.na(pond_indc)) %>% 
  distinct(ident_ind, .keep_all = TRUE) %>%
  as_survey_design(ids = ident_ind,
                   weights = pond_indc)  %>% 
